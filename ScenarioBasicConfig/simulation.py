...
def robot_failure_process(
    ...
    # Wait until the selected robot failure time.
    yield env.timeout(failure_time)

    # Permanently disable the failed robot.
    robot.failed = True
    ...
    record_event(
        ...
        robot_id=robot.robot_id,
        status=(
            "Robot failed and remains unavailable "
    ...
    # If no helper is selected, stop here.
    if not helper_enabled:
        return

    # Wait until the selected helper activation time.
    delay_until_helper = (
        helper_activation_time - env.now
    )

    if delay_until_helper > 0:
        yield env.timeout(
            delay_until_helper
        )

    # Select an existing robot as the helper.
    helper_robot = next(
        existing_robot
        for existing_robot in robots
        if (
            existing_robot.robot_id
            == helper_robot_id
        )
    )

    state["helper_robot_id"] = (
        helper_robot.robot_id
    )
    ...
    record_event(
        ...
        time=env.now,
        strategy=strategy,
        robot_id=helper_robot.robot_id,
        status=(
            f"Robot {helper_robot.robot_id} "
            f"starts helping after Robot "
            f"{robot.robot_id} failed"
        ),
        position=helper_robot.position,
    )
...
def run_simulation(
    ...
    failed_robot_id = failure_config.get(
        "robot_id"
    )

    failure_time = failure_config.get(
        "failure_time"
    )
    ...
    helper_robot_id = failure_config.get(
        "helper_robot_id"
    )

    helper_activation_time = failure_config.get(
        "helper_activation_time"
    )
    ...
    if failure_enabled:
        ...
        if failed_robot_id not in range(
            ...
            raise ValueError(
                "The selected failed robot does "
                "not exist."
            )
        ...
        if helper_enabled:
            ...
            helper_activation_time = float(
                helper_activation_time
            )

            if helper_robot_id not in range(
                1,
                number_of_robots + 1,
            ):
                raise ValueError(
                    "The selected helper robot "
                    "does not exist."
                )
            ...
            if helper_activation_time < failure_time:
                raise ValueError(
                    "Helper activation time cannot "
                    "be earlier than failure time."
                )
    ...
    if is_custom_scenario:
        ...
        for (
            robot_id,
            position,
        ) in configured_positions.items():
            converted_robot_id = int(
                robot_id
            )
        ...
        if actual_robot_ids != expected_robot_ids:
            raise ValueError(
                "Custom robot_start_positions must "
                "contain exactly one position for "
                "every robot. "
                f"Expected: "
                f"{sorted(expected_robot_ids)}. "
                f"Received: "
                f"{sorted(actual_robot_ids)}."
            )
